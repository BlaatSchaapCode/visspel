...
t::Game make_game(types::PlayerID player_count) {
    ...
    for (size_t i = 0; i < map.m_tiles.size(); ++i) {
        ...
        // TODO: Coastline randomisation
        if (border_distance < border_size)
            tile = t::TileType::Grass;
        else if (border_distance < border_size + 1)
            tile = t::TileType::ShallowWater; // TODO: Water should be placed around the landmass as a post-processing. This works better when adding islands later, I guess.
        else
